...
export default class PostgresDB implements SQLPreparedStatement {
    ...
    public async getIndicesForTable(tableName: string): Promise<string[]> {
      ...
      FROM pg_indexes WHERE tablename = \'${tableName.toLowerCase()}\'`)).map((result: any) => result.name as string);
    }
    public async getAllTriggers(): Promise<string[]> {
        return (await this.all(sql`SELECT tgname AS name FROM pg_trigger,pg_proc WHERE
      pg_proc.oid=pg_trigger.tgfoid AND tgisinternal = false`)).map((result: any) => result.name as string);
    }
